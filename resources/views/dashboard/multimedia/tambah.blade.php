...
<section>
    ...
    <div class="col-lg-12">
        <h3>Input Data</h3>
        <form action="/dashboard/multimedia/tambah-data" method="POST" enctype="multipart/form-data">
            @csrf
            <!-- Date -->
            ...
            <div class="input-group mb-3">
                <input id="link" type="text" name="link_video" class="form-control @error('link_video') is-invalid @enderror" placeholder="Link Video" aria-label="Link Video" width="50%" value="{{ old('link_video') }}" required>
                @error('link_video')
                    <div class="invalid-feedback">
                        {{ $message }}
                    </div>
